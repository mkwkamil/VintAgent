...
check("created 7 URLs", len(created) == 7, len(created))
...
for url_id in created:
    ...
    if status == 200:
        ...
        rejected += 1
check("only 6 threads may start", started == 6 and rejected == 1, f"started={started} rejected={rejected}")

status, body = call("GET", "/api/stats", token=token)
check("stats report 6 active threads", status == 200 and body["active_threads"] == 6, body)
...
check("6 URLs marked running", len(running) == 6, len(running))
check("all running threads survived failing polls", len(alive) == 6, len(alive))
check("failed polls were recorded", len(checked) == 6, [u["last_error"] for u in running][:2])

with open(DATA_FILE, encoding="utf-8") as fh:
    persisted = json.load(fh)
check("data.json holds 7 records", len(persisted["urls"]) == 7, len(persisted["urls"]))
check(
    "data.json keeps status + error per record",
    sum(1 for r in persisted["urls"] if r["status"] == "running") == 6
...
status, body = call("POST", f"/api/urls/{created[6]}/start", token=token)
check("a freed slot lets the 7th URL start", status == 200, (status, body))
...
status, body = call("GET", "/api/urls", token=token)
check("deleted URL is gone", status == 200 and len(body) == 6, len(body) if status == 200 else body)

status, body = call("GET", "/api/stats", token=token)
check("thread count follows start/stop/delete", status == 200 and body["active_threads"] == 5, body)
